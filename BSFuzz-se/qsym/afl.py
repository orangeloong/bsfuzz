...
class AFLExecutor(object):
    ...
    def run_file(self, fp):
        ...
        '''
        if os.path.exists(q.testcase_directory+"/unsolved.csv"):
            with open(self.output+"/unsolved.csv", 'a') as df:
                with open(q.testcase_directory+"/unsolved.csv", 'r') as csvfile: 
            ...
            os.unlink(q.testcase_directory+"/unsolved.csv")
        '''
        ...
        for testcase in q.get_testcases():
            ...
            testcase_filename = os.path.basename(testcase)
            filename = os.path.join(
                self.my_queue,
                "id:%06d,src:%s,qsym:%s" % (index, target, testcase_filename))
            shutil.move(testcase, filename)
            logger.debug("Creating: %s" % filename)
            # logger.debug("From testcase: %s" % testcase)
            """
            # use addr2line to find the symbolic branch in source code
            flag = testcase_filename.split("flag:")
            if len(flag) == 1:
                continue
            flag_list = flag[1].split("-")
            testcase_pc = flag_list[0]
            target_addr = flag_list[-1]
            # logger.debug(testcase_pc)

            target_binary = self.cmd[0]
            # addr2line
            cmd0 = ["addr2line", "-e", target_binary, testcase_pc, "-f"]
            logger.debug(cmd0)
            proc0 = subprocess.Popen(cmd0, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)

            out0, err0 = proc0.communicate()
            # get results
            function_name = (out0.splitlines())[0]
            line_info = ((out0.splitlines())[1].rsplit(" (discriminator"))[0]
            logger.debug("jcc in function: "+function_name+",line info: "+line_info)

            # addr2line
            cmd1 = ["addr2line", "-e", target_binary, target_addr, "-f"]
            logger.debug(cmd1)
            proc1 = subprocess.Popen(cmd1, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)

            out1, err1 = proc1.communicate()
            # get results
            target_function_name = (out1.splitlines())[0]
            target_line_info = ((out1.splitlines())[1].rsplit(" (discriminator"))[0]
            logger.debug("target addr in function: "+target_function_name+",line info: "+target_line_info)

            # if not os.path.exists(self.output+'/symbolicBranch.csv'):
            #    os.makedirs(self.output+'/symbolicBranch.csv')

            # save results
            # result = "func:"+function_name+",line:"+line_info
            branchFile_path = self.output+'/symbolicBranch.csv'
            with open(branchFile_path, 'a+') as f:
                # filename
                csv_writer = csv.writer(f)
                csv_writer.writerow([function_name, line_info, target_line_info, os.getcwd()+'/'+filename])
                # f.write(os.getcwd()+'/'+filename+"\n")
                f.close()
            """
