...
namespace qsym
{
  ...
  Solver::Solver(
      ...
      : input_file_(input_file), inputs_(), out_dir_(out_dir), context_(g_z3_context), solver_(z3::solver(context_, "QF_BV")), num_generated_(0), trace_(bitmap), last_interested_(false), syncing_(false), start_time_(getTimeStamp()), solving_time_(0), last_pc_(0), dep_forest_()
  {
  ...
  z3::check_result Solver::check()
  {
    ...
    z3::check_result res;
    LOG_STAT(
        "SMT: { \"solving_time\": " + decstr(solving_time_) + ", " + "\"total_time\": " + decstr(before - start_time_) + " }\n");
  ...
  bool Solver::checkAndSave(const std::string &postfix, const std::string &taken)
  {
    if (check() == z3::sat)
    {
      saveValues(postfix, taken);
      return true;
    }
    else
    {
  ...
  void Solver::addJcc(ExprRef e, bool taken, ADDRINT pc, ADDRINT target)
  {
    ...
    if (e->isConcrete())
    {
      LOG_DEBUG("e is concrete: " + e->toString() + "\n");
      return;
    }
    ...
    if (e->kind() == Bool)
    {
      ...
      LOG_DEBUG("e's kind is bool: " + e->toString() + "\n");
      return;
    }
    ...
    {
      // add target addr
      is_interesting = isInterestingJcc(e, taken, pc, target, afl_inv_idx, afl_idx);
      LOG_DEBUG("Add Jcc: " + hexstr(pc) + "\n");
    }
    if (is_interesting)
    {
      negatePath(e, taken, target, afl_inv_idx, afl_idx);
      LOG_DEBUG("This Jcc is interesting: " + hexstr(pc) + "\n");
    }
    LOG_DEBUG("addConstraint: " + e->toString() + "\n");
  ...
  void Solver::closeOutFile()
  {
    //outFile.close();
  }

  void Solver::prev_jcc(ADDRINT afl_h)
  {
    prev_jcc_ = afl_h;
  }
  ...
  void Solver::checkOutDir()
  {
    ...
    //outFile.open(std::string(out_dir_) + std::string("/unsolved.csv"), std::ios::out);
  }
  ...
  void Solver::saveValues(const std::string &postfix, const std::string &taken)
  {
  ...
  bool Solver::isInterestingJcc(ExprRef rel_expr, bool taken, ADDRINT pc, ADDRINT target, ADDRINT &afl_inv_idx, ADDRINT &afl_idx)
  {
    bool interesting = trace_.isInterestingBranch(pc, target, taken, afl_inv_idx, afl_idx, prev_jcc_);
    // record for other decision
    last_interested_ = interesting;
    return interesting;
  }

  void Solver::negatePath(ExprRef e, bool taken, ADDRINT target, ADDRINT &afl_inv_idx, ADDRINT &afl_idx)
  {
    reset();
    syncConstraints(e);
    addToSolver(e, !taken);
    ...
    if (taken)
    {
      iftaken = "false-" + hexstr(target);
    }
    else
    {
      iftaken = "true-" + hexstr(target);
    }
    */

    bool sat = checkAndSave("", iftaken);
    if (!sat)
    {
      reset();
      // optimistic solving
      addToSolver(e, !taken);
      sat = checkAndSave("optimistic", iftaken);

      if (!sat)
      {
        trace_.isUnsolvedBranch(afl_inv_idx);
        /*
        outFile << hexstr(last_pc_) << ','
                << hexstr(target) << ','
                << (taken
                        ? "false"
                        : "true")
