...
namespace qsym
{
  ...
  XXH32_hash_t hashPc1(ADDRINT pc, bool taken, IMG img)
  {
      ...
      REG r)
  {
    ...
    LOG_INFO("Silent concretization at " + hexstr(pc) + ": Instruction=" + disassemble(pc) + ", REG=" + REG_StringShort(r) + ", Expr=" + e->toString() + "\n");
      ...
      UINT32 size)
  {
    ...
    ADDRINT pc = PIN_GetContextReg(ctx, REG_INST_PTR);
    LOG_INFO("Silent concretization at " + hexstr(pc) + ": Instruction=" + disassemble(pc) + ", Memory=" + hexstr(addr) + ", Size=" + decstr(size) + ", Expr=" + e->toString() + "\n");
      ...
      ExprRef *expr_src)
  {
    ...
    *expr_src = thread_ctx->getExprFromReg(ctx, src);
    ...
    if (*expr_dst == NULL && *expr_src == NULL)
      return false;
      ...
      ExprRef *expr_cf)
  {
    ...
    *expr_cf = thread_ctx->computeCfAsExpr(ctx, bits);
    ...
    if (!getExprFromRegReg(thread_ctx, ctx, dst, src, expr_dst, expr_src) && *expr_cf == NULL)
      return false;
      ...
      ExprRef *expr_cf)
  {
    ...
    *expr_cf = thread_ctx->computeCfAsExpr(ctx, bits);
    ...
    if (!getExprFromRegImm(thread_ctx, ctx, dst, imm, expr_dst, expr_src) && *expr_cf == NULL)
      return false;
      ...
      ExprRef *expr_src2)
  {
    *expr_src2 = thread_ctx->getExprFromReg(ctx, src2);
    ...
    if (!getExprFromRegReg(thread_ctx, ctx, dst, src1, expr_dst, expr_src1) && *expr_src2 == NULL)
      return false;
                          ...
                          const CONTEXT *ctx, REG dst, REG src)
  {
    ...
    if (expr_dst == NULL && expr_src == NULL && expr_ax == NULL)
    {
      thread_ctx->invalidateEflags(CC_OP_SUB);
      return;
    }
                ...
                JccKind jcc_c, bool inv)
  {
...
#endif
      g_solver->addJcc(e, taken, pc, target);
    }
    ...
    if(target != 0)
    {
      ...
      if(IMG_Valid(img))
        g_solver->prev_jcc(hashPc1(pc, taken, img));
      //}
  ...
  doVinsert128i(ExprRef expr_src1, ExprRef expr_src2, ADDRINT imm)
  {
    ...
    QSYM_ASSERT(expr_src1->bits() == 256 && expr_src2->bits() == 128);
